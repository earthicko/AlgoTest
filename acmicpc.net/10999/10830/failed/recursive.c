...
int** power_matrix(int** base, long long power) {
    if (DEBUG) printf("P %lld: Start\n", power);
    if (power == 1) {
        if (DEBUG) printf("Return copy of base (power = 1)\n");
        int** output = copy_matrix(base);
        return output;
    }

    int** half_result = power_matrix(base, power / 2);
    if (DEBUG) {
        printf("P %lld: half result from P %lld\n", power, power / 2);
        print_matrix(half_result);
    }

    if (power % 2 == 1) {
        int** half_result_2 = copy_matrix(half_result);
        int** temp = multiply_matrix(half_result, half_result_2);
        int** result = multiply_matrix(temp, base);

        if (DEBUG) {
            printf("P %lld: temp from P %lld\n", power, (power / 2) * 2);
            print_matrix(temp);
        }

        free_matrix(half_result);
        free_matrix(half_result_2);
        free_matrix(temp);
        ...
        int** half_result_2 = copy_matrix(half_result);
        int** result = multiply_matrix(half_result, half_result_2);
